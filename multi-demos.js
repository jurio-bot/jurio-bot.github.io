...
if(crRun&&crFault&&crReset){
 let running=false,withFault=true;
 const variants=qa('.variant'),steps=qa('.creative-step'),qaItems=qa('.qa-item');
 const scores=[94,91,89,87,85,82,79,77,74,71,69,66];
 function crResetAll(){running=false;withFault=true;setState('#creative-state','READY / BRIEF LOADED');steps.forEach(s=>s.className='creative-step');variants.forEach((v,i)=>{v.className='variant';v.dataset.score=`${scores[i]}`});qaItems.forEach(x=>x.className='qa-item');const pack=q('#creative-pack');if(pack)pack.classList.remove('done');const prog=q('#creative-progress');if(prog)prog.style.width='0';clearLog('#creative-log');logTo('#creative-log','waiting / 12 variants / QA rules loaded','');crFault.textContent='不良ありで回す';disable([crRun,crFault,crReset],false)}
