...
async function deliver(job,{ackLoss=false}={}){
 ...
 await packet($(`worker-current-${job}`)||$('station-workers'),qa,job,'green',390);log(`${job} / QA passed`,'ok');await wait(180);
...
async function job1042(chaos){
 ...
 await packet($(`queue-${job}`),station('station-router'),job,'',360);await packet($('station-router'),$('worker-a'),job,'',360);worker('worker-a','busy','RUNNING');$('worker-current-J-1042').dataset.live='1';log(`${job} / worker A started`);await wait(430);
 if(chaos){worker('worker-a','failed','DOWN');subsystem('health-card','A DOWN','warn');status('DEGRADED / RECOVERING','alert');log(`${job} / worker A heartbeat lost`,'warn');await wait(260);addMetric('retry');subsystem('retry-card','1 / 2 USED','warn');log(`${job} / effect not committed / retry budget consumed`,'gold');worker('worker-c','awake','AWAKE');log(`${job} / reserve worker C activated`,'ok');await packet($('worker-a'),$('worker-c'),job,'red',520);$('worker-current-J-1042').id='worker-current-J-1042-old';const proxy=document.createElement('span');proxy.id='worker-current-J-1042';proxy.hidden=true;$('worker-c').appendChild(proxy);await wait(260);await deliver(job);subsystem('health-card','A DOWN / C ACTIVE','warn');}
...
async function job1043(chaos){
 ...
 await packet($(`queue-${job}`),station('station-router'),job,'gold',360);await packet($('station-router'),$('worker-b'),job,'gold',360);worker('worker-b','busy','RUNNING');$('worker-current-J-1043').dataset.live='1';log(`${job} / worker B started`);await wait(300);
...
async function job1044(){
 ...
 await packet($(`queue-${job}`),station('station-router'),job,'green',340);const target=$('worker-b');await packet($('station-router'),target,job,'green',340);log(`${job} / worker B reused without waiting for A`);const proxy=$('worker-current-J-1044');if(proxy)proxy.dataset.live='1';await wait(310);await deliver(job)
}

function reset(){running=false;startAt=performance.now();Object.keys(metrics).forEach(k=>setMetric(k,k==='queued'?3:0));status('READY / 3 JOBS QUEUED');['station-router','station-qa','station-delivery','station-ledger'].forEach(id=>station(id,false));worker('worker-a','','READY');worker('worker-b','','READY');worker('worker-c','standby','STANDBY');subsystem('health-card','ALL HEALTHY');subsystem('guard-card','0 BLOCKED');subsystem('retry-card','2 / 2 AVAILABLE');['J-1042','J-1043','J-1044'].forEach(j=>queueState(j,''));const logBox=$('ops-log');if(logBox)logBox.innerHTML='<p class="muted">waiting / choose a run mode</p>';const receipts=$('receipt-list');if(receipts)receipts.innerHTML='<div class="receipt pending"><i></i><span>waiting for completed jobs</span><span class="hash">—</span></div>';chaosBtn.disabled=false;normalBtn.disabled=false;resetBtn.disabled=false}
